...
function SearchResults() {
    ...
    function sortResults(criteria){
        ...
        if(criteria==="Price"){
            ...
            sortedBuses=busesCopy.sort((a,b)=>{
                ...
                return -1;
                else if(a.departureTime.includes("PM") && b.departureTime.includes("AM"))
                return 1;
                else{
                    const matchesA=a.departureTime.match(/\d+/g);
                    const matchesB=b.departureTime.match(/\d+/g);
                    return parseInt(Number(matchesA[0]+matchesA[1]))-parseInt(Number(matchesB[0]+matchesB[1]));
                }
            })
        }
        else{
            sortedBuses=busesCopy.sort((a,b)=>{
                ...
                return 1;
                    ...
                    return parseInt(matchesA[0]+matchesA[1])-parseInt(matchesB[0]+matchesB[1]);
                }
            })
        }
    ...
    return (
        <div className='p-2 bg-danger d-flex flex-column'>
                ...
                <div className='d-flex justify-content-around w-100'>
                    {["Departure","Arrival","Price"].map(criteria=>{
                        return (
                            ...
                            }} variant="danger" className="rounded-0">{criteria}</Button>
                        )
                    })}
                </div>
            </div>
